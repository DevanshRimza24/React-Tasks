...
const Login = () => {
    ...
    const handleLogin = async () => {
        try {
            ...
            const response = await axios.post("http://localhost:3001/api/login", { userName : username, password }, { withCredentials: true });
    ...
    const validateForm = () => {
        ...
        if (!formData.username.trim()) {
            ...
            newErrors.username = "Name must be atleast 3 characters"
        }
    ...
    const validateInput = (name: string, value: string | boolean) => {
        ...
        switch (name) {
            case "username":
                newErrors.username = value.toString().trim().length < 3 ? "Name must be atleast 3 characters" : ""
                break
        ...
        <div className='bg-gray-100 rounded-lg shadow-md max-w-lg mx-auto p-6'>
            ...
            <input type="text" name="name" value={formData.name} onChange={handleChange} className='input bg-gray-50'/>
             ...
             {errors.name && <p className='error'>{errors.name}</p>}
                ...
                <div className='flex flex-col mb-4 justify-between'>
                    <div className="flex justify-between">
                        <label className='block font-medium mr-10'>Username</label>
                        <input type="username" name="username" value={formData.username} onChange={handleChange} className='input bg-gray-50' />
